...
function FloatingTierCard({ card }: { card: FloatCard }) {
  ...
  return (
    <div
      className="absolute flex items-center gap-3 min-w-55 rounded-2xl border border-white/10 bg-[#16161C] p-2.5 pr-4 shadow-[0_12px_40px_rgba(0,0,0,0.55)]"
      style={{ top, left, zIndex, transform: `rotate(${rotate})` }}
    >
      <div
        className={`flex shrink-0 items-center justify-center w-13 h-13 rounded-sm border text-3xl font-extrabold tracking-wide ${tier.text} ${tier.bg} ${tier.border}`}
      >
        {tier.label}
      </div>
      <div className="flex flex-1 flex-row gap-1.5">
        {Array.from({ length: skeletons }).map((_, i) => (
          <div key={i} className="size-11 rounded-sm bg-secondary" />
        ))}
      </div>
    </div>
  );
...
export function TierIllustration() {
  return (
    <div
      className="relative shrink-0 w-140 h-110"
      style={{
        maskImage:
